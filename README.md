...
The shared backend is a free Neon project with PostgreSQL and Functions. `neon.ts` declares the function. The manual **Deploy eBay deletion endpoint** GitHub Actions workflow deploys from `main` to the Finder Neon Production branch. It reads the Production eBay key pair from repository secrets and reads `NEON_API_KEY` (scoped to this Neon project) and `EBAY_DELETION_VERIFICATION_TOKEN` from the GitHub `production` environment. It passes the exact public function URL as `EBAY_DELETION_ENDPOINT_URL`. The eBay key pair and Neon key reach only the deployment step; the verification token also reaches the post-deployment challenge check. Raw deployment output is withheld from public Actions logs. The workflow checks eBay's GET challenge and rejection of an unsigned POST. The Production key pair and verification token are stored in Neon's Function environment; no local credential file is needed for this cloud workflow. An optional local deployment can use an untracked `.env.local` with the same four values and `neon deploy --env .env.local --no-env-pull`; never commit that file. The exact URL and same verification token must be saved in eBay Developer Portal → Production → Alerts & Notifications → Marketplace Account Deletion. After the workflow passes, send eBay's test notification and confirm the endpoint acknowledges it. Until that succeeds, do not run real Production scans.
